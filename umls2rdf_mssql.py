...
class UmlsClass(object):
    ...
    def code(self):
        ...
        if len(codes) != 1:
            raise RuntimeError("Only one code per term.")
        return codes.pop()
    ...
    def getPrefLabel(self):
        if self.load_on_cuis:
            ...
            raise RuntimeError("Unable to select pref label")
...
class UmlsAttribute(object):
    ...
    def toRDF(self,dockey,desc,fmt="Turtle"):
        ...
        if "REL" in dockey:
            ...
            raise RuntimeError("Unknown DOCKEY" + dockey)
            ...
            if self.load_on_cuis:
                ...
                for rel in rels:
                    ...
                    if len(code_source) != 1 or len(code_target) > 1:
                        raise RuntimeError("more than one or none codes")
    ...
    def write_properties(self,fout,property_docs):
        ...
        for p in self.ont_properties:
            ...
            if "expanded_form" not in doc:
                raise RuntimeError("expanded form not found in " + doc)
